...
const PROMPT_TEMPLATE = `
        ...
        ## 핵심 규칙
        1. **매일 다른 운세 생성**
          - {{today}} 날짜를 기반으로 매일 완전히 다른 순위와 내용 생성
          - 날짜마다 무작위성을 최대한 활용하여 다양한 결과 생성
          - 특정 패턴이나 규칙성 없이 자연스럽게 변화

        2. **순위 다양성**
          - 1~12위를 매일 완전히 다르게 배정
          - 특정 별자리가 반복적으로 상위/하위권 고정 금지
          - 각 별자리는 동등한 확률로 모든 순위 가능

        3. **내용 다양성**
          - 5가지 테마 중 선택: 대인관계/업무학업/감정내면/실용생활/기회변화
          - 동일 키워드를 3개 이상 별자리에서 반복 금지
          - 일상적이고 공감 가능한 조언으로 작성

        4. **작성 스타일**
          - 2~3문장, 밝고 귀여운 TV 톤
          - 마지막 문장은 구체적 행동 팁으로 마무리
        ...
        필수사항:
        - 정확히 12개 (양자리,황소자리,쌍둥이자리,게자리,사자자리,처녀자리,천칭자리,전갈자리,사수자리,염소자리,물병자리,물고기자리)
